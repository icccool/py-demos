...
def mem_info():
    child = ssh_command(server_info["username"], server_info["ip"], server_info["password"], "cat /proc/meminfo")
...
def vm_stat_info():
    child = ssh_command(server_info["username"], server_info["ip"], server_info["password"], "vmstat 1 2 | tail -n 1")
    child.expect(pexpect.EOF)
    ...
    "*******************时间：", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "******************"
    print
    "等待运行进程的数量:", processes_waiting
    print
    "处于不间断状态的进程:", processes_sleep
    print
    "使用虚拟内存(swap)的总量:", swpd
    print
    "空闲的内存总量:", free
    print
    "用作缓冲的内存总量:", buff
    ...
    "发送到块设备:", io_bo
    print
    "每秒的中断数:", system_interrupt
    print
    "每秒的上下文切换数:", system_context_switch
    print
    "用户空间上进程运行的时间百分比:", cpu_user
    print
    "内核空间上进程运行的时间百分比:", cpu_sys
    print
    "闲置时间百分比:", cpu_idle
    print
    "等待IO的时间百分比:", cpu_wait
...
def cpu_info():
    child = ssh_command(server_info["username"], server_info["ip"], server_info["password"], "cat /proc/cpuinfo")
    child.expect(pexpect.EOF)
    ...
    for processor in CPUinfo.keys():
        print
        "CPU属于的名字及其编号、标称主频:", CPUinfo[processor]['model name']
        print
        "CPU属于其系列中的哪一代的代号:", CPUinfo[processor]['model']
        print
        "CPU制造商:", CPUinfo[processor]['vendor_id']
        print
        "CPU产品系列代号:", CPUinfo[processor]['cpu family']
        print
        "CPU的实际使用主频:", CPUinfo[processor]['cpu MHz']
...
def load_stat():
    child = ssh_command(server_info["username"], server_info["ip"], server_info["password"], "cat /proc/loadavg")
    child.expect(pexpect.EOF)
    ...
    "*******************时间：", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "******************"
    print
    "系统5分钟前的平均负载：", loadavgs[0]
    print
    "系统10分钟前的平均负载：", loadavgs[1]
    print
    "系统15分钟前的平均负载：", loadavgs[2]
    print
    "分子是正在运行的进程数,分母为总进程数：", loadavgs[3]
    print
    "最近运行的进程id：", loadavgs[4]
...
def ionetwork():
    print("11")
    # child = ssh_command(server_info["username"], server_info["ip"], server_info["password"], "cat /proc/net/dev")
    # child.expect(pexpect.EOF)
...
def disk_stat():
    child = ssh_command(server_info["username"], server_info["ip"], server_info["password"], "df -h")
    child.expect(pexpect.EOF)
    disk = child.before
...
def getComStr():
    child = ssh_command(server_info["username"], server_info["ip"], server_info["password"], "netstat -tpln")
    child.expect(pexpect.EOF)
    Com = child.before
    print
    '******************************端口监控*********************************'
...
"""
def cpu():
    child = ssh_command(server_info["username"], server_info["ip"], server_info["password"], 'cat /proc/stat | grep "cpu "')
    child.expect(pexpect.EOF)
    child1 = ssh_command(server_info["username"], server_info["ip"], server_info["password"], 'cat /proc/stat | grep "cpu "')
